 ...
 * 
 */
async function tradeByClientAddressesInDifferentParticipant() {
   ...
   */
  const participantReceiverAccount = '<Endereço do Participante onde o Cessionário (receiverAccount) de TPFt pertence>';
  ...
  setApprovalForAll(senderAccount, tpftAddress, tpftDvpAddress) 
    ...
    * de acordo com os critérios do participante do cliente (cessionário).
    */
  const realDigitalAmount = 1000;
 
  approveRealDigital(realDigitalAddress, participantReceiverAccount, realDigitalAmount, tpftDvpAddress, swapOneStepFromAddress)
