 ...
 * realizem a operação de compromissada na ida com preço unitário de retorno aberto 
 * envolvendo Título Público Federal tokenizado (TPFt) entre si utilizando seus CNPJ8s.
 ...
 */
async function tradeRepoOpenReturnPriceByCNPJ8() {
  /**
  ...
  const params = {
    ...
    returnDate: "<Data de retorno em segundos do TPFt (timestamp Unix)>",
  };
  ...
  const senderTransaction = await TPFtOperation1054
    .connect(sender)
    ?.[
      "tradeRepoOpenReturnPrice(uint256,uint256,uint256,uint8,(string,string,uint256),uint256,uint256,uint256)"
    ](
      params.operationId,
      params.cnpj8Sender,
...
/**
 * TPFtOperation1054 - Permite que participantes cadastrados no Real Digital 
 * realizem a operação de compromissada intraday na ida com preço unitário de retorno aberto 
 * envolvendo Título Público Federal tokenizado (TPFt) entre si utilizando seus endereços de carteiras.
 * @dev Para a operação ocorrer é necessário que cada carteira de aprovação para o endereço contrato TPFtDvP
 * no Real Digital através da função approve e no TPFt através setApprovalForAll.
 */
async function tradeRepoOpenReturnPriceByAddresses() {
  ...
  const senderTransaction = await TPFtOperation1054
    .connect(sender)
    ?.[
      "tradeRepoOpenReturnPrice(uint256,address,address,uint8,(string,string,uint256),uint256,uint256,uint256)"
    ](
      params.operationId,
      sender.address,
      receiver.address,
 ...
 * realizem a operação de compromissada na ida com preço unitário de retorno aberto
 * envolvendo Título Público Federal tokenizado (TPFt) entre si utilizando seus endereços 
 * de carteiras e seus respetivos endereços de Real Tokenizado.
 * @dev Para a operação ocorrer é necessário que cada carteira de aprovação para o endereço contrato TPFtDvP
 * no Real Digital através da função approve e no TPFt através setApprovalForAll.
 */
async function tradeRepoOpenReturnPriceByAddressesParticipantClient() {
  ...
  const senderTransaction = await TPFtOperation1054
    .connect(sender)
    ?.[
      "tradeRepoOpenReturnPrice(uint256,address,address,address,address,uint8,(string,string,uint256),uint256,uint256,uint256)"
    ](
      params.operationId,
      senderAddress,
